...
def update_figure(current_ax, current_fig):
    ...
    final_path = np.loadtxt(PLAN_PATH, delimiter=',')
    current_ax.plot(final_path[:,0], final_path[:,1], color=color, linewidth=2.0)
...
def plot_map_and_path():
    ...
    # plot map border
    ax.plot(xs, ys, '-k', lw=4)
    ax.plot(final_path[:,0], final_path[:,1], 'b-', linewidth=2.0)
    ax.axis("equal")
    return ax, fig
